...
unsigned long int odmacierzA; 
                     odmacierzA = recv(sock,&macierz_A,sizeof(macierz_A),0);  //odebranie od klienta macierzy A
                 if( odmacierzA < 0)
                    puts("ERROR recv()");
                 if(odmacierzA == 0)
                    puts("ERROR zostalo zamkniete polaczenie");
                 if( odmacierzA == macierz_w_bajtach )
                    puts("Potwierdzenie odbioru macierzy A\n");
                 else if ( odmacierzA != macierz_w_bajtach )
                    puts("Blad potwierdzenia odbioru macierzy A\n");
...
unsigned long int odmacierzB;
                    odmacierzB =  recv(sock,&macierz_B,sizeof(macierz_B),0);  //odebranie od klienta macierzy B
                if( odmacierzB < 0)
                  puts("ERROR recv()");
                if(odmacierzB == 0)
                  puts("ERROR zostalo zamkniete polaczenie");
                if( odmacierzB ==  macierz_w_bajtach )
                  puts("Potwierdzenie odbioru macierzy B\n");
                else if( odmacierzB !=  macierz_w_bajtach )
                  puts("Blad potwierdzenia odbioru macierzy B\n");
 ...
 for(i=0;i<rm[0];i++)
 {
  ...
  unsigned long int potwierdzenie_dla_klienta[2]; //dane dla klienta - odebrane w bajtach macierze A oraz B
  
  potwierdzenie_dla_klienta[0] = odmacierzA;
  potwierdzenie_dla_klienta[1] = odmacierzB;
  
  if( send(sock, &potwierdzenie_dla_klienta, sizeof(potwierdzenie_dla_klienta),0) < 0)   //wysłanie potwierdzenia do klienta
    {
        puts("Blad wysylania");
        //return 1;
    }
